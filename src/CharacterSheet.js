...
class CharacterSheet extends React.PureComponent {
  ...
  render() {
    ...
    const prioData = char.hasOwnProperty('priorities') ? char.priorities.data : {};
    return (
      <div id="rb-character-sheet" className={Classes.DARK}>
        ...
        <CharacterTabs character={char} setCharacter={this.setCharacter}>
          ...
          <Tab id="gameoptsel" title="Gameplay Options" panel={
            <GameOptionsPanel 
            ...
            />} 
          />
          <Tab id="priosel" title="Priority Selection" panel={
            <PrioSelPanel 
            ...
            />} 
          />
          <Tab id="talentsel" title="Talent" panel={
            <TalentSelPanel
            ...
            />} 
          />
          <Tab id="metatypesel" title="Metatype" panel={
            <MetatypePanel 
            ...
            />} 
          />
          <Tab id="attr" title="Attributes" panel={
            <AttrPanel 
  ...
  initCharacter() {
    ...
    this.props.children.forEach(child => {
      if (child.props.panel.type.prototype.hasOwnProperty('initPanel')) {
        let { hooks, ...updatedProps } = child.props.panel.type.prototype.initPanel(char);
        if ( hooks ) {
  ...
  render() {
    return (
      <Callout className="rb-remaining-box" intent={(this.props.karmaRemaining >= 0) ? null : "warning"}>
        <span><span>Karma Remaining:</span> {this.props.karmaRemaining}</span>
      </Callout>
    );
  }
}
